"""
STL Protocol — Sentiment-To-Logic confidence calibration.

Ported from Sentinel-X (sentinel-x/python/council/stl_protocol.py).
...
- Penalises contradictions between agents.
- Detects shallow reasoning (arguments without specific data points).
- Caps confidence when regime is not tradeable.
"""
...
@dataclass
class STLResult:
    """Calibrated output of the STL Protocol."""
...
async def run_stl_protocol(
    ...
    """Run the STL Protocol across all 5 agent arguments.

    Uses the intelligent scheduler to route the LLM call through the GPU
    circuit breaker, falling back to a heuristic calibration on CPU.
    """
    debate_input = (
        f"Symbol: {symbol}\n\n"
    ...
    def _cpu_fallback() -> dict:
        """Heuristic calibration when GPU circuit breaker is open."""
        bull_score = bull_arg.get("quantitative_score", 0.5)
        bear_score = bear_arg.get("quantitative_score", 0.5)
        quant_tradeable = quant.get("tradeable", True)

        base_conf = max(bull_score, bear_score) * 100.0
        if not quant_tradeable:
            base_conf = min(base_conf, 60.0)

        n_contradictions = 0
        # Simple contradiction heuristic: same-side supporting factors vs risks
        if bull_arg.get("supporting_factors") and bear_arg.get("supporting_factors"):
            n_contradictions = 1
        base_conf -= n_contradictions * 5

        return {
            "bull_score": bull_score,
            "bear_score": bear_score,
            "confidence_pct": round(max(min(base_conf, 100.0), 0.0), 1),
            "final_side": "BUY" if bull_score > bear_score else ("SELL" if bear_score > bull_score else "HOLD"),
            "blocked": base_conf < 75.0,
            "block_reason": f"CPU fallback: confidence {base_conf:.1f}%" if base_conf < 75.0 else "",
            "agent_weights": {},
            "contradictions": [],
            "rationale": "CPU fallback calibration (circuit breaker open).",
        }

    result = await route(
        TaskType.LLM,
        gpu_fn=_gpu_calibrate,
        cpu_fn=_cpu_fallback,
    )
