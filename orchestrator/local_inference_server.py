...
try:
    ...
    VLLM_INFERENCE_LATENCY = Histogram(
        "vllm_inference_latency_ms",
        "vLLM inference latency in milliseconds",
        ["model"],
        buckets=[10, 25, 50, 100, 200, 500, 1000, 2000, 5000],
    )
    VLLM_TOKENS_GENERATED = Counter(
        "vllm_tokens_generated_total",
        "Total tokens generated by vLLM",
        ["model"],
    )
    VLLM_REQUESTS_TOTAL = Counter(
        "vllm_requests_total",
        "Total vLLM inference requests",
        ["model", "endpoint"],  # endpoint: /generate, /generate/fast, /generate/stream
    )
    VLLM_ACTIVE_REQUESTS = Gauge(
        "vllm_active_requests",
        "Currently processing vLLM requests",
    )
    VLLM_PROMETHEUS_AVAILABLE = True
except ImportError:
    VLLM_PROMETHEUS_AVAILABLE = False
...
def create_app(config: Optional[LocalInferenceConfig] = None):
    ...
    async def generate(req: GenerateRequest):
        ...
        try:
            if VLLM_PROMETHEUS_AVAILABLE:
                VLLM_ACTIVE_REQUESTS.inc()
                VLLM_REQUESTS_TOTAL.labels(model=cfg.model, endpoint="/generate").inc()
            ...
            if VLLM_PROMETHEUS_AVAILABLE:
                VLLM_INFERENCE_LATENCY.labels(model=model_name).observe(latency_ms)
                VLLM_TOKENS_GENERATED.labels(model=model_name).inc(tokens)
